...
function ApplicationModel(stompClient) {
  ...
  self.cfmtp = ko.observable(new CFMTPModel());
  ...
  self.connect = function() {
      ...
      stompClient.subscribe("/topic/tradeMessages", function(message) {
        self.cfmtp().processQuote(JSON.parse(message.body));
      });
...
function CFMTPModel() {
  ...
  self.loadPositions = function(positions) {
    for ( var i = 0; i < positions.length; i++) {
      var row = new CFMTPRow(positions[i]);
      self.rows.push(row);
